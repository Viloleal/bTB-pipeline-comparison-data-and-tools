...
for (files,prefix) in zip (vcfs,prefixes):
	...
	df2 = df1[df1.b != 'N'] 
	dfx, remove = prox(10)
	print ("Removed %s positions" % (len(dfx)))
	dfx = dfx[['pos']]
	removed_list = dfx['pos'].to_list()
	print(len(removed_list))
	df1 = df1[~df1.pos.isin(removed_list)]
	df1.to_csv('%s_processed.vcf' % (prefix), sep='\t', header=False, index=False)
	remove.to_csv('%s_removed.vcf' % (prefix), header=True,index=False)
	file_object = open('%s_removed.vcf' % (prefix),'a')
	file_object.write("Removed %s positions" % (len(removed_list)))
